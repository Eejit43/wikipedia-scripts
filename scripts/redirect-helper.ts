...
.redirect-helper-autofix-button a {
    ...
    min-height: unset !important;
            ...
            private loadInputElements() {
                ...
                this.redirectInput = new RedirectTargetInputWidget(
                    { placeholder: 'Target page name', required: true },
                ...
                this.tagSelect.on('change', () => {
                    const sortedTags = (this.tagSelect.getValue() as string[]).sort((a, b) =>
                ...
                this.categorySelect.on('change', () => {
                    const sortedTags = (this.categorySelect.getValue() as string[]).sort((a, b) =>
                        a.toLowerCase().localeCompare(b.toLowerCase()),
                    );

                    if ((this.categorySelect.getValue() as string[]).join(';') !== sortedTags.join(';'))
                        this.categorySelect.setValue(sortedTags);
            ...
            private async validateSubmission() {
                ...
                /* Self redirects */
                if (this.parsedDestination?.toString() === this.pageTitleParsed.toString())
                ...
                if (destinationParseResult.parse.redirects?.[0]) {
                    const destinationRedirect =
                        ...
                        (destinationParseResult.parse.redirects[0].tofragment
                            ? `#${destinationParseResult.parse.redirects[0].tofragment}`
                            : '');
                    errors.push({
                        title: destination,
                ...
                if (destination.split('#').length > 1) {
                    ...
                    if (validSection) {
                        ...
                        const anchors = [
                            ...
                            ...(destinationContent
                                .match(/EpisodeNumber += +\d+/g)
                                ?.map((anchor: string) => `ep${anchor.split('=')[1].trim()}`) ?? []),
                        ];
                ...
                if (targetIsDisambiguationPage && !taggedAsRedirectToDisambiguationPage && !taggedAsRedirectToSurnameList)
                    errors.push({
                        message:
                            'is a redirect to a disambiguation page, but it is not tagged with a disambiguation categorization template!',
                    });
                ...
                if (destinationData!.query.pages[0].pageprops && !targetIsDisambiguationPage) {
                    ...
                    if (
                        ...
                        errors.push({
                            message:
                                'is not a redirect to a disambiguation page, but it is tagged with a disambiguation categorization template!',
                    ...
                    if (targetIsSurnameList && !taggedAsRedirectToSurnameList)
                        errors.push({
                            message:
                                'is a redirect to a surname list, but it is not tagged with a correct disambiguation categorization template!',
                        });
                }
                ...
                if (
                    this.syncTalkCheckbox?.isSelected() &&
            ...
            private async handleSubmitButtonClick() {
                        ...
                        if (autoFixes) {
                            ...
                            autoFixButton.on('click', () => {
                                ...
                                for (const autoFix of autoFixes) {
                                    if (autoFix.type === 'add' && !tags.includes(autoFix.tag))
                                        this.tagSelect.addTag(autoFix.tag, autoFix.tag);

                                    if (autoFix.type === 'remove' && tags.includes(autoFix.tag))
                                        this.tagSelect.removeTagByData(autoFix.tag);
                ...
                const summary =
                    (this.summaryInput.getValue() || (this.summaryInput.$tabIndexed[0] as HTMLInputElement).placeholder) +
                    this.scriptAdvert;
